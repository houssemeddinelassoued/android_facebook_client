...
public class NetController {
	...
	public void getNameAndPicture(Activity activity, String id,
	...
	public void getLatestStatus(Activity activity, String id,
			final RequestObserver<DAOMessage> observer) {
		Bundle b = new Bundle();
		b.putString("limit", "1");
		b.putString("fields", "message");
		FacebookRequest r = new FacebookRequest(activity, id + "/statuses", b,
				facebookController, new FacebookRequest.Observer() {
					@Override
					public void onError(Exception ex) {
						observer.onError(ex);
					...
					public void onComplete(FacebookRequest facebookRequest) {
						try {
							...
							for (int i = 0; i < friendArray.length(); ++i) {
								...
								friendList.add(new DAONameAndPicture(id, name,
										picture));
							}
	...
	public void getNewsFeed(Activity activity,
		...
		FacebookRequest r = new FacebookRequest(activity, "me/home", b,
				facebookController, new FacebookRequest.Observer() {
					...
					public void onComplete(FacebookRequest facebookRequest) {
						try {
							JSONArray feedItems = facebookRequest.getResponse()
									.getJSONArray("data");
							Vector<DAONewsFeedItem> feedList = new Vector<DAONewsFeedItem>();
							for (int i = 0; i < feedItems.length(); ++i) {
								...
								String fromId = item.getJSONObject("from")
										.getString("id");
								String fromName = item.getJSONObject("from")
										.getString("name");
								String message = item
										.optString("message", null);
								String picture = item
										.optString("picture", null);
								String name = item.optString("name", null);
								String description = item.optString(
										"description", null);
								String createdTime = item.optString(
										"created_time", null);
								feedList.add(new DAONewsFeedItem(id, type, fromId,
										fromName, message, picture, name,
	...
	public void getBitmap(Activity activity, String url,
